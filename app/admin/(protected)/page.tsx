...
export default async function AdminDashboardPage() {
  ...
  // Run all the DB calls in parallel
  const [
    totalEmployees,
    activeEmployees,
    totalLocations,
    ...
    prisma.shift.count({
      where: {
        ...
        location: {
          // We treat a location with code "ADHOC" as adhoc
          code: "ADHOC",
        },
      },
    }),
      ...
      <section className="grid gap-4 grid-cols-1 md:grid-cols-3">
        <div className="rounded-xl border bg-white p-4 shadow-sm col-span-1">
          ...
          <p className="mt-3 text-xs text-gray-500">
            Use the <span className="font-medium">Shifts</span> page to review
            and map ADHOC clock-ins for compliance checks.
          </p>
        </div>
                    ...
                    return (
                      <tr key={s.id} className="border-b last:border-0">
                        <td className="py-1 pr-4">
                          {s.user?.name || s.user?.employeeCode || "Unknown"}
                        </td>
                        <td className="py-1 pr-4">
